...
resource "aws_security_group_rule" "access_tcp_from_internet" {
  ...
  description       = "Allow from internet to tcp port ${element(var.allow_ports, count.index)}"
  protocol          = "tcp"
...
resource "aws_security_group_rule" "access_udp_from_internet" {
  ...
  description       = "Allow from internet to udp port ${element(var.allow_ports, count.index)}"
  protocol          = "tcp"
  cidr_blocks       = ["0.0.0.0/0"]
}
